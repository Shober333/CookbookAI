...
|---|------|--------|---------------------|
| 1.1 | Initialize Next.js 15 project with TypeScript, Tailwind CSS, App Router | `[ ]` | `npm run dev` starts at `localhost:3000` with no errors |
| 1.2 | Install and configure shadcn/ui | `[ ]` | `Button`, `Input`, `Card` components render correctly |
| 1.3 | Set up Prisma with SQLite; define schema (User, Recipe, EquipmentProfile, Auth.js tables) | `[ ]` | `npx prisma migrate dev` runs without errors; `dev.db` created |
| 1.4 | Install and configure Auth.js v5 with Prisma adapter and credentials provider | `[ ]` | Auth.js handler at `/api/auth/[...nextauth]` returns 200 |
| 1.5 | Install Anthropic SDK and Vercel AI SDK; create `src/lib/anthropic.ts` with client singleton | `[ ]` | File exports `anthropic` client; env var `ANTHROPIC_API_KEY` documented in `.env.example` |
| 1.6 | Create `src/lib/db.ts` Prisma client singleton | `[ ]` | No "PrismaClient instantiated multiple times" warnings in dev |
| 1.7 | Configure route middleware to protect `(app)` routes | `[ ]` | Unauthenticated request to `/library` redirects to `/login` |
...
| # | Task | Status | Acceptance Criteria |
|---|------|--------|---------------------|
| 3.1 | Write Claude system prompt for recipe extraction; output must be a JSON object matching the `Recipe` type | `[ ]` | Prompt reviewed and saved in `src/lib/anthropic.ts`; system prompt uses Anthropic prompt caching (`cache_control`) |
| 3.2 | `/api/ai/import` route — fetch URL content server-side, send to Claude via `streamText`, stream structured recipe JSON back | `[ ]` | POST with a valid recipe URL returns a streaming response; final JSON contains `title`, `servings`, `ingredients[]`, `steps[]` |
...
| 3.5 | `/api/recipes` POST endpoint — validate payload, insert Recipe row | `[ ]` | Returns `201` with the saved recipe; rejects unauthenticated requests with `401` |
...
| # | Task | Status | Acceptance Criteria |
|---|------|--------|---------------------|
| 4.1 | `/api/recipes` GET endpoint — return all recipes for authenticated user | `[ ]` | Returns array of recipes; no cross-user leakage; empty array (not 404) when library is empty |
| 4.2 | `/api/recipes/[id]` GET endpoint — return single recipe (owner check) | `[ ]` | Returns recipe for owner; returns `403` for non-owner; `404` for missing ID |
| 4.3 | `/api/recipes/[id]` DELETE endpoint | `[ ]` | Deletes recipe; returns `200`; non-owner gets `403` |
