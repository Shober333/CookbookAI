...
Codex support lives in `.codex/`: role prompts in `.codex/roles/` and
workflow prompt starters in `.codex/prompts/`. `AGENTS.md` remains the
canonical role constitution for both Claude and Codex.
...
│   ├── roles/                 # Codex role prompts mapped to AGENTS.md
│   └── prompts/               # Codex workflow prompt starters
│
├── backend/
